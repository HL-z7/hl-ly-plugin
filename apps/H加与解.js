...
export class jyj extends plugin {
  ...
  async jiaurl(e) {
    let textToEncode = e.msg.replace(/^Hurl编码/, "").trim();
  ...
  async jieurl(e) {
    let textToDecode = e.msg.replace(/^Hurl解码/, "").trim();
  ...
  async encodeBase64(e) {
    let textToEncode = e.msg.replace(/^Hbase64编码/, "").trim();
  ...
  async decodeBase64(e) {
    let textToDecode = e.msg.replace(/^Hbase64解码/, "").trim();
  ...
  async encodeHexToBase64(e) {
    let textToEncode = e.msg.replace(/^Hb64编码/, "").trim();
  ...
  async decodeBase64ToHex(e) {
    let textToDecode = e.msg.replace(/^Hb64解码/, "").trim();
    // 过滤非 Base64 字符
  ...
  async encodeMD5(e) {
    let textToEncode = e.msg.replace(/^Hmd5编码/, "").trim();
  ...
  async encodeUnicode(e) {
    let textToEncode = e.msg.replace(/^HUnicode编码/, "").trim();
  ...
  async decodeUnicode(e) {
    let textToDecode = e.msg.replace(/^HUnicode解码/, "").trim();
